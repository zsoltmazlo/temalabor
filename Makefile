...
ifeq ($(OS),Windows_NT)
    $(info Building on Windows platform...)
    ...
    UNAME:=$(shell uname)
    ifeq ($(UNAME),Linux)
        $(info Building on Linux platform...)
        $(info ${PATH})
        # Base directory for the compiler
        ...
        SDK_TOOLS	?= /opt/Espressif/utils

        # esptool path and port
        ESPTOOL ?= python /opt/Espressif/esptool/esptool.py
        ESPPORT ?= /dev/ttyUSB*
    else 
$(error Unknown platform, halting.)
    endif
endif
...
ifeq ($(OS),Windows_NT)
	...
	BAUD ?= 115200 #need it? - yes (@zsoltmazlo)
endif
...
CFLAGS = -Os -g -Os -std=gnu90 -Wpointer-arith -Wundef -Werror -Wl,-EL -fno-inline-functions -nostdlib -mlongcalls -mtext-section-literals -mno-serialize-volatile -D__ets__ -DICACHE_FLASH
